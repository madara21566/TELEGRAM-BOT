...
def dashboard():
    ...
    return render_template_string(html, uptime=uptime)render_template_string("""
<!doctype html>
<html lang="en"><head>
<meta charset="utf-8" />
    ...
    if request.method == 'POST':
        ...
        async def do_broadcast():
            ...
            for uid in audience:
                try:
                    if kind == 'text':
                        await tg_bot.send_message(chat_id=uid, text=text)
                    elif kind == 'photo' and file:
                        file.stream.seek(0)
                        await tg_bot.send_photo(chat_id=uid, photo=InputFile(file.stream, filename=file.filename), caption=text or None)
                    elif kind == 'document' and file:
                        file.stream.seek(0)
                        await tg_bot.send_document(chat_id=uid, document=InputFile(file.stream, filename=file.filename), caption=text or None)
                    sent += 1
                except Exception:
                    continue
            ...
            with sqlite3.connect(DB_FILE) as conn:
                ...
                c.execute("INSERT INTO broadcasts (admin, kind, content, created_at) VALUES (?, ?, ?, ?)",
                          (session.get("admin_user"), kind,
                           text if kind=='text' else (file.filename if file else ''),
                           datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
                conn.commit()

        threading.Thread(target=lambda: application.create_task(do_broadcast())).start()
        note = f"📢 Broadcast started to {len(audience)} users. Sending in background..."
