...
# ---------------------- ADVANCED ADDITIONS ----------------------
# The following block adds:
# - Edit/Replace flow (editing_sessions)
# - VCF -> TXT converter (command + inline callback)
# - Auto-clean and DB backup background tasks
# - Minimal aiohttp web dashboard endpoint (/dashboard and /health)
# NOTE: This is appended to the original file. If there are duplicate symbols, you may need to merge manually.
editing_sessions = {}

async def start_background_tasks(dp=None):
    async def auto_clean_task():
        while True:
            try:
                cutoff_days = int(os.getenv("AUTO_CLEAN_DAYS", "30"))
                cutoff = datetime.now() - timedelta(days=cutoff_days)
                for uid, files in list(user_files.items()):
                    removed = []
                    for fname, ftype in list(files):
                        user_folder = UPLOAD_BOTS_DIR / str(uid)
                        p = user_folder / fname
                        if p.exists():
                            ...
                            if mtime < cutoff:
                                try:
                                    ...
                                    pass
                                removed.append((fname, ftype))
                    if removed:
                        user_files[uid] = [f for f in user_files[uid] if f not in removed]
                        try:
                            conn = sqlite3.connect(DATABASE_PATH)
                            c = conn.cursor()
                            for fname, _ in removed:
                ...
                try:
                    logger.exception("Auto-clean error: %s", e)
                except:
                    pass
    ...
    async def db_backup_task():
        while True:
            try:
                ...
                conn.backup(bconn)
                bconn.close(); conn.close()
                # keep last 7 days
                for bf in BACKUP_DIR.glob("backup_*.db"):
                    try:
                        if datetime.fromtimestamp(bf.stat().st_mtime) < datetime.now() - timedelta(days=7):
                            bf.unlink(missing_ok=True)
                    except:
                        pass
            except Exception as e:
                try:
                    logger.exception("DB backup error: %s", e)
                except:
                    pass
            await asyncio.sleep(int(os.getenv("DB_BACKUP_HOURS", "24"))*3600)
...
@dp.callback_query(F.data and F.data.startswith("convert_vcf:"))
async def callback_convert_vcf(cb: types.CallbackQuery):
    user_id = cb.from_user.id
    file_name = cb.data.split(":",1)[1]
    user_folder = UPLOAD_BOTS_DIR / str(user_id)
    vcf_path = user_folder / file_name
    if not vcf_path.exists():
        await cb.answer("VCF not found", show_alert=True); return
    try:
        txt_name = file_name.rsplit(".",1)[0] + ".txt"
        out_path = user_folder / txt_name
        with open(vcf_path, "r", encoding="utf-8", errors="ignore") as rf, open(out_path, "w", encoding="utf-8") as wf:
            for line in rf:
                line = line.strip()
                if line.startswith("FN:") or line.startswith("TEL:") or line.startswith("TEL;") or line.startswith("N:"):
                    wf.write(line + "\n")
        # register
        now = datetime.now().isoformat()
        if user_id not in user_files:
            user_files[user_id] = []
        user_files[user_id].append((txt_name, "txt"))
        conn = sqlite3.connect(DATABASE_PATH)
        c = conn.cursor()
        c.execute('INSERT OR REPLACE INTO user_files (user_id, file_name, file_type, upload_date) VALUES (?, ?, ?, ?)', (user_id, txt_name, "txt", now))
        conn.commit(); conn.close()
        await cb.message.answer(f"Converted {file_name} → {txt_name}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📁 My Files", callback_data="check_files")]]))
        await cb.answer("VCF converted to TXT", show_alert=True)
    except Exception as e:
        logger.exception("VCF convert error: %s", e)
        await cb.answer(f"Error: {e}", show_alert=True)

# Minimal web dashboard using aiohttp if main app uses it
try:
    ...
    def setup_dashboard(app):
        async def health(request):
            ...
            return _web_mod.json_response(data)
        app.router.add_get("/health", health)
        async def dashboard_page(request):
            html = f"<html><body><h1>Bot Dashboard</h1><pre>{json.dumps({'users': len(active_users),'uploads':bot_stats.get('total_uploads',0)}, indent=2)}</pre></body></html>"
            return _web_mod.Response(text=html, content_type="text/html")
        app.router.add_get("/dashboard", dashboard_page)
except Exception as e:
    pass

# Hook into startup if possible
try:
    # if there's a start_web_app or on_startup, schedule tasks after short delay
...
# ---------------------- KEEP-ALIVE PINGER (Render 24/7) ----------------------
import aiohttp as _aiohttp

async def keep_alive_task():
    url = os.getenv("KEEPALIVE_URL")
    if not url:
        # No keepalive URL configured; skip
        return
    while True:
        try:
            async with _aiohttp.ClientSession() as session:
                async with session.get(url, timeout=10) as resp:
                    # optional: read or log status
                    await resp.read()
        except Exception as _e:
            try:
                logger.debug(f"Keep-alive ping failed: {_e}")
            except:
                pass
        await asyncio.sleep(300)  # 5 minutes

# Schedule keep-alive on startup if possible
try:
    asyncio.create_task(keep_alive_task())
except Exception:
    # If create_task fails at import-time, schedule it on startup in on_startup if exists
    try:
        async def _schedule_keepalive():
            await asyncio.sleep(2)
            asyncio.create_task(keep_alive_task())
        asyncio.create_task(_schedule_keepalive())
    except Exception:
        pass
