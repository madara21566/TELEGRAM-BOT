    ...
    if user_id in conversion_mode:
        ...
        if mode == "txt2vcf" and path.endswith(".txt"):
            ...
            if numbers:
                ...
                await update.message.reply_text("âŒ No numbers found in TXT file.")
            ...
            if numbers:
                ...
                await update.message.reply_text("âŒ No numbers found in VCF file.")
        else:
            await update.message.reply_text("âŒ Wrong file type for this command.")
...
async def my_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    settings = (
        f"ðŸ“‚ File name: {user_file_names.get(user_id, default_vcf_name)}\n"
        f"ðŸ‘¤ Contact name: {user_contact_names.get(user_id, default_contact_name)}\n"
        f"ðŸ“Š Limit: {user_limits.get(user_id, default_limit)}\n"
        f"ðŸ”¢ Start index: {user_start_indexes.get(user_id, 'Not set')}\n"
        f"ðŸ“„ VCF start: {user_vcf_start_numbers.get(user_id, 'Not set')}\n"
        f"ðŸŒ Country code: {user_country_codes.get(user_id, 'None')}\n"
        f"ðŸ“‘ Group start: {user_group_start_numbers.get(user_id, 'Not set')}"
    )
...
async def merge_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"ðŸ“‚ Send me files to merge. Final file: {merge_data[user_id]['filename']}.vcf\nðŸ‘‰ Use /done when finished.")

async def done_merge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id not in merge_data or not merge_data[user_id]["files"]:
        await update.message.reply_text("âŒ No files queued for merge.")
        return
    all_numbers = set()
