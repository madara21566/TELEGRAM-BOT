...
async def my_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_key(update):
        # If they don't have a valid key, still allow showing if they have one but invalid?
        # We'll show if they have a stored key even if expired/invalid, but check_key already replies access message.
        user_id = update.effective_user.id
        key = user_keys.get(user_id)
        if key:
            expiry = valid_keys.get(key)
            exp_text = "♾ Permanent" if expiry is None else expiry.strftime("%Y-%m-%d %H:%M:%S UTC")
            await update.message.reply_text(f"🔑 Your key: {key}\n⏳ Expires: {exp_text}")
        return
    user_id = update.effective_user.id
    key = user_keys.get(user_id)
    expiry = valid_keys.get(key)
