...
from utils.helpers import load_json, save_json, ensure_state_user, STATE_FILE, is_premium
...
def register_project_handlers(dp, bot, owner_id, base_url):
    ...
    async def receive_name(msg: types.Message):
        ...
        if st.get("awaiting_name", {}).get(str(msg.from_user.id)):
            ...
            # Enforce project count limits (Free=2, Premium=10)
            is_prem = is_premium(msg.from_user.id)
