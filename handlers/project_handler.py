...
def register_project_handlers(dp, bot, owner_id, base_url):
    ...
    # -------------- DOCUMENT HANDLER --------------
    # Yahi project upload + backup upload dono handle karega
    ...
    async def receive_doc(msg: types.Message):
        ...
        # 🔥 1) OWNER BACKUP UPLOAD MODE
        if st.get("awaiting_backup_upload") and uid == OWNER_ID:
            ...
            return  # backup handle ho gaya, project upload mat samajh
        ...
        # Requirements install ko background thread me daal rahe hain
        ...
        # 🎉 3) Spam "upload complete" control (30 sec per user)
        st = load_json()
        user_state = st.setdefault("users", {}).setdefault(str(uid), {})
        last_ts = user_state.get("last_upload_ts", 0)
        now = time.time()
        if now - last_ts > 30:
            ...
            user_state["last_upload_ts"] = now
            save_json(STATE_FILE, st)
        # agar 30 sec ke andar bohot files bhejega, to sirf ek hi success msg aayega.
