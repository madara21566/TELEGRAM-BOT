...
def register_project_handlers(dp, bot, owner_id, base_url):
    ...
    async def receive_name(msg: types.Message):
        ...
        await msg.answer(
            f"✅ Project `{name}` created.\nNow send your .py or .zip as *DOCUMENT*.",
            parse_mode="Markdown",
        )
    ...
    def _extract_zip(zip_path, dest):
        ...
        # single inner-folder ko flatten karo
        while True:
    ...
    async def receive_doc(msg: types.Message):
        # 🔥 IMPORTANT:
        # Agar admin "Upload & Restore" backup mode me hai,
        # to ye handler backup zip ko touch NHI karega.
        st = load_json()
        if st.get("awaiting_backup_upload") and msg.from_user.id == OWNER_ID:
            # isko admin_handler ka handle_backup_upload handle karega
            return
