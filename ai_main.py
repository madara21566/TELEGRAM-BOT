...
def home():
    ...
    <p>ðŸ•’ Uptime: {{ uptime }}</p>
    <p>ðŸ‘¥ Users: {{ users }} | ðŸ“ Files: {{ files }}</p>
    <p><a href='/admin'>ðŸ” Admin Panel</a></p>
...
def admin_login():
    if request.method == 'POST':
        ...
        return "âŒ Wrong password"
    return '''<form method=post><h3>ðŸ” Admin Login</h3>Password: <input type=password name=password><input type=submit value=Login></form>'''
...
def admin_dashboard():
    ...
    <ul>
        <li><a href="/admin/live-logs">ðŸ“º Live Logs</a></li>
        <li><a href="/admin/access">ðŸ” Access Control</a></li>
        <li><a href="/admin/logout">ðŸšª Logout</a></li>
    </ul>
    '''
...
def access_panel():
    ...
    {% for r in rows %}
        <tr><td>{{r[0]}}</td><td>{{r[1]}}</td><td>{{r[2]}}</td><td>{{r[3] or 'âˆž'}}</td>
        <td><a href='/admin/delaccess?uid={{r[0]}}'>âŒ</a></td></tr>
    {% endfor %}
    </table>
...
def protected(handler_func, command_name):
    async def wrapper(update, context):
        ...
        if not is_authorized(user.id):
            await update.message.reply_text("âŒ You don't have access to use this bot.")
            return
