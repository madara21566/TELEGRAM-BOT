...
def get_dashboard_data():
    ...
    if os.path.exists("data/backups"):
        ...
        if zips:
            zips.sort(key=lambda x: os.path.getmtime(os.path.join("data/backups",x)), reverse=True)
            last_backup = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(os.path.getmtime(os.path.join("data/backups",zips[0]))))
    return {
