...
def start_script(uid, proj, cmd=None):
    ...
    if not entry and not cmd: raise RuntimeError("No entry .py found")
    if not cmd: cmd = f"python3 {entry}"
    ...
    st.setdefault("procs",{}).setdefault(suid,{})[f"{proj}:entry"] = {"pid": proc.pid, "start": int(time.time()), "cmd": cmd}
...
def read_logs(uid, proj, lines=500):
    ...
    with open(path,"r",encoding="utf-8",errors="ignore") as f: data = f.readlines()
