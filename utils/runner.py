...
def _project_dir(uid, proj): return f"{LOGS_DIR}/{uid}/{proj}"

# robust entry discovery even after ZIPs
def pick_entry_py(base):
    for root, dirs, files in os.walk(base):
        if "main.py" in files: return os.path.join(root, "main.py")
        for f in files:
            if f.endswith(".py"): return os.path.join(root, f)
    return None

def start_script(uid, proj, cmd=None):
    base = _project_dir(uid, proj); os.makedirs(base, exist_ok=True)
    entry = pick_entry_py(base)
    if not entry and not cmd: raise RuntimeError("No Python entry file found")
    if not cmd: cmd = f"python3 {os.path.basename(entry)}"

    stop_script(uid, proj)  # ensure only one instance

    log_path = os.path.join(base, "logs.txt")
    logf = open(log_path, "a", buffering=1, encoding="utf-8", errors="ignore")
    proc = subprocess.Popen(cmd, shell=True, cwd=os.path.dirname(entry),
...
def stop_script(uid, proj):
    key = (uid, proj); proc = processes.get(key)
...
def restart_script(uid, proj, cmd=None):
    stop_script(uid, proj); time.sleep(0.5); return start_script(uid, proj, cmd)

def get_status(uid, proj):
    proc = processes.get((uid, proj))
    if not proc: return {"running": False, "pid": None}
