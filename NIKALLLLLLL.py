...
ALLOWED_USERS = [2134530726,7440046924,6105744293,8128934569,7950732287,7669357884,8047407478,7193035541,7118726445,5849097477,7938117492,6190086618]  # ✅ Add more IDs here if needed
...
async def merge_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Merge session started. Send VCF or TXT files.\n"
        f"Final file will be: {output_name}.vcf\n"
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id in merge_data:
        if file_ext == 'vcf':
            ...
            await update.message.reply_text("Only VCF and TXT supported.")
            os.remove(path)
            return
