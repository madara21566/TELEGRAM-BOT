...
async def make_vcf_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # remove duplicates & keep only digits
    numbers = list(dict.fromkeys([n.strip() for n in numbers if n.strip().isdigit()]))
    if not numbers:
        await update.message.reply_text("❌ No valid numbers given.")
        return
...
async def process_numbers(update, context, numbers):
    ...
    for idx, chunk in enumerate(chunks):
        group_num = None if user_id not in user_group_start_numbers else custom_group_start + idx
